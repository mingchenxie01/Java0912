 ...
 */
class Test21{
    //   static void dummyMethod() throws ClassNotFoundException{
//        try{
//            System.out.println(Class.forName("Day2"));
//        }catch(ClassNotFoundException ex){
//            System.out.println("class ex happened");
//        }catch(ArrayIndexOutOfBoundsException ex){
//            System.out.println("Array ex happened");
//        }catch(Exception ex){//Exception是所有其他exception的总和，所以必须放在其他exception之下，因为Exception可以抓到其他所有的exception
//            //放在第一位的话，之后就不需要其他的exception，这种就是checked exception 是在compile的时候就出现
//            System.out.println("ex");
//        }finally{
//            //no matter ex happened or not
//            System.out.println("this is finally");
//        }
        static void dummy(){
            int[] arr = new int[1];
            try{
                System.out.println(arr[1]);//这种exception就是unchecked exception是发生在run time
                //这种exception在compile的时候可以通过，但是在run的时候会出现error
            }catch(Exception ex){
                System.out.println(ex);
            }
        }
 ...
 *      advantage: addLast time:O(1)
 *      add(index, item) O(n) traverse O(1) manipulate pointer
 *      disadvantage: get() Time complexity:O(n) :traverse through first/last O(n) next...next prev.prev
 *  ArrayList Object[]
 ...
 *      get()
 *      put()
 *      Key.hashcode() -> rehash(hc) -> rehash value % length of node array (n - 1) & hash == hash % n -> index find
 *      the target bucket -> hash collison
 *
 *      HashMap
 *          Node(key, value, next)
 *          put -> putVal -> (hash(key), key, value)
 *              1. check whether tab is empty
 *              2. if there is no collision, we just creat new node and put into the bucket
 ...
 *                  if we traverse all the node, and cannot find a hit, we are gonna create a newNode and append it at
 *                  then end
